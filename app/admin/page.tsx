...
import { getProjects, getAllTransactions, getAllChats, deleteProject, getDanaUsages } from '@/lib/firestore';
import Badge, { getStatusBadge } from '@/components/ui/Badge';
import Button from '@/components/ui/Button';
import toast from 'react-hot-toast';
import DanaUsageModal from '@/components/admin/DanaUsageModal';
import type { Project, Transaction, Chat, DanaUsage } from '@/types';
...
export default function AdminPage() {
  ...
  useEffect(() => {
    Promise.all([getProjects(), getAllTransactions(), getAllChats(), getDanaUsages()])
      .then(([p, t, c, d]) => {
        setProjects(p);
        setTransactions(t);
        setChats(c);
      ...
      <div className="flex space-x-2 border-b border-gray-200">
        <button
          ...
          className={`pb-3 px-4 text-sm font-medium border-b-2 transition-colors ${
            activeTab === 'projects' ? 'border-teal-600 text-teal-600' : 'border-transparent text-gray-500 hover:text-gray-700'
          }`}
        >
          Daftar Proyek
        </button>
        <button
          ...
          className={`pb-3 px-4 text-sm font-medium border-b-2 transition-colors ${
            activeTab === 'transactions' ? 'border-teal-600 text-teal-600' : 'border-transparent text-gray-500 hover:text-gray-700'
          }`}
        >
          Riwayat Transaksi
        </button>
        <button
          ...
          className={`pb-3 px-4 text-sm font-medium border-b-2 transition-colors ${
            activeTab === 'dana_usage' ? 'border-teal-600 text-teal-600' : 'border-transparent text-gray-500 hover:text-gray-700'
          }`}
        >
          Penggunaan Dana
        </button>
